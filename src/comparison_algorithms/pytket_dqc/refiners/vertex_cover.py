...
from comparison_algorithms.pytket_dqc.refiners.refiner import Refiner
from comparison_algorithms.pytket_dqc.packing.pacman import PacMan, MergedPacket
from comparison_algorithms.pytket_dqc.placement.placement import Placement
from comparison_algorithms.pytket_dqc.circuits.hypergraph import Hyperedge
...
class VertexCover(Refiner):
    ...
    def exhaustive_refine(
        ...
        for subgraph in [
            ...
            min_covers: List[List[MergedPacket]] = get_min_covers(list(subgraph.edges))
...
def get_min_covers(edges: List[tuple[Any, Any]]) -> List[List[Any]]:
    ...
    def get_covers(edges: List[tuple[Any, Any]]) -> List[List[Any]]:
        ...
        if not edges:
            ...
            for c in covers_w_v1:
                c.add(v1)
            # Return the union
            # NOTE: I'm not using List[List[Any]] instead of List[List[Any]]
            # due to set not being hashable -> I'd need List[frozenset[Any]]
