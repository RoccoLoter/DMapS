...
from comparison_algorithms.pytket_dqc.allocators.allocator import Allocator
from comparison_algorithms.pytket_dqc.allocators.gain_manager import GainManager
...
class HypergraphPartitioning(Allocator):
    ...
    def allocate(self, circ: Circuit, network: NISQNetwork, **kwargs) -> Distribution:
        ...
        default_ini = f"{package_path}/km1_kKaHyPar_sea20.ini"
        ...
        **kwargs,
        ...
        if len(dist_circ.hyperedge_list) == 0:
            ...
            vertex_weights = [1 for i in range(0, num_qubits)] + [
                1 for i in range(num_qubits, num_vertices)
            ]
