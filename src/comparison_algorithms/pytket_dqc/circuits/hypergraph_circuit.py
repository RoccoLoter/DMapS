...
from comparison_algorithms.pytket_dqc.circuits.hypergraph import (
    Hypergraph,
    Hyperedge,
    Vertex,
)
from comparison_algorithms.pytket_dqc.utils.gateset import (
...
from typing import TYPE_CHECKING, Union, Optional, cast, Dict, List, Tuple
...
class HypergraphCircuit(Hypergraph):
    ...
    def from_dict(
        ...
        hypergraph_circuit = cls(Circuit.from_dict(hypergraph_circuit_dict["circuit"]))
        ...
        for next_hyp_gate_vertex in gate_vertices[1:]:
            ...
            if not h_indices:
                ...
                subcirc_commands += prepare_h_embedding(
                    embedded_commands[h_indices[0] : h_indices[-1] + 1]  # noqa: E203
                )
                # All embedded commands after the last Hadamard
                # are D-embedded
                subcirc_commands += embedded_commands[h_indices[-1] + 1 :]  # noqa: E203
    ...
    def is_h_embeddable_CU1(
        ...
        return np.isclose(command.op.params[0] % 1, 0) or np.isclose(
            command.op.params[0] % 1, 1
        )
